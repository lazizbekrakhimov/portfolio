      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="flex gap-4 mb-12 flex-wrap">
          {(['advanced', 'medium', 'basic'] as const).map(level => (
            <button key={level} onClick={() => setActiveLevel(activeLevel === level ? null : level)} className="flex items-center gap-2 px-4 py-2 transition-all text-[0.65rem] tracking-widest uppercase" style={{ border: `1px solid ${activeLevel === level ? levelColors[level] : 'var(--border)'}`, color: activeLevel === level ? levelColors[level] : 'var(--muted)', background: activeLevel === level ? 'var(--blue-dim)' : 'transparent', }} >
              <span className="w-2 h-2 rounded-full" style={{ background: levelColors[level], boxShadow: level === 'advanced' ? '0 0 6px var(--accent)' : 'none', }} />
              {levelLabels[level]}
            </button>
          ))}
        </div>
